...
class IframeUtiltites {
    ...
    static lightCacheClear() {
        try {
            ...
            if ('caches' in window) {
                ...
                caches.keys().then((cacheNames) => {
                    const essentialCaches = cacheNames.filter((name) => name.includes('rocketfuel') || name.includes('age-verification'));
    ...
    static attemptRecovery(context, _error) {
        try {
            switch (context) {
                case 'showOverlay':
 ...
 */
function getDefaultIframeService() {
    return {
        get iframe() {
            return IframeUtiltites.iframe;
        },
        showOverlay: (url, feature) => IframeUtiltites.showOverlay(url, feature),
        closeIframe: () => IframeUtiltites.closeIframe(),
        setIframeHeight: (height) => IframeUtiltites.setIframeHeight(height),
    };
}
...
class ApiClient {
    ...
    encrypt(data, clientId) {
        return CryptoJS__default["default"].AES.encrypt(data, clientId).toString();
    }
...
class RKFLPlugin {
    constructor(config, iframeService) {
        var _a, _b;
        this.redirect = false;
        this.payNowButton = null;
        this.zkpInitialized = false;
        ...
        this.redirect = (_a = config.redirect) !== null && _a !== void 0 ? _a : false;
        this.environment = (_b = config.environment) !== null && _b !== void 0 ? _b : 'production';
        this.uuid = '';
        this.userInfo = { email: '', userId: '' };
        this.iframeService = iframeService !== null && iframeService !== void 0 ? iframeService : getDefaultIframeService();
    }
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.clientId) {
            ...
            this.buttons.forEach((btnType) => {
                ...
                switch (btnType.feature) {
                    case FEATURE_PAYIN.feature: {
                        ...
                        const container = document.getElementById((_a = btnType.containerId) !== null && _a !== void 0 ? _a : ContainerId);
                        if (!container) {
                            console.error('Container not found.');
                            return;
                        }
                        button.onclick = () => __awaiter(this, void 0, void 0, function* () {
                            if (this.iframeService.iframe)
                                return;
                            if (!this.uuid) {
                            ...
                            try {
                                const { placeOrder } = yield Promise.resolve().then(function () { return require('./index-6cf9d7d8.cjs.js'); });
                                yield placeOrder(this.clientId, this.redirect, this.uuid, this.environment);
                            }
                            catch (err) {
                    ...
                    case FEATURE_AGE_VERIFICATION.feature: {
                        ...
                        button.id = '#age';
                        const container2 = document.getElementById((_b = btnType.containerId) !== null && _b !== void 0 ? _b : ContainerId);
                        if (!container2 && btnType.inject) {
                            console.error('Container not found.');
                            return;
                        }
                        button.onclick = () => __awaiter(this, void 0, void 0, function* () {
                            const { initializeWidget, launchAgeVerificationWidget } = yield Promise.resolve().then(function () { return require('./index-b524d685.cjs.js'); });
    ...
    handleMessage(event) {
        ...
        if (data.type === 'initialize_widget') {
            ...
            if (((_a = this.iframeService.iframe) === null || _a === void 0 ? void 0 : _a.contentWindow) && access) {
                this.iframeService.iframe.contentWindow.postMessage({ type: 'initialize_widget', data: payload }, '*');
            }
        }
        if (data.type === 'rocketfuel_change_height') {
            this.iframeService.setIframeHeight(data.data);
            (_c = (_b = this.iframeService.iframe) === null || _b === void 0 ? void 0 : _b.contentWindow) === null || _c === void 0 ? void 0 : _c.postMessage({ type: 'rocketfuel_return_width', data: window.outerWidth }, '*');
        }
    }
    ...
    launchPaymentWidget(uuid) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const { placeOrder } = yield Promise.resolve().then(function () { return require('./index-6cf9d7d8.cjs.js'); });
                yield placeOrder(this.clientId, this.redirect, uuid, this.environment);
            }
            catch (err) {
                console.error('Error during order placement:', err);
            }
...
exports.FEATURE_AGE_VERIFICATION = FEATURE_AGE_VERIFICATION;
exports.FEATURE_PAYIN = FEATURE_PAYIN;
exports.IframeUtiltites = IframeUtiltites;
exports.RKFLPlugin = RKFLPlugin;
exports.__awaiter = __awaiter;
exports.appDomains = appDomains;
